  ...
  <div class="slider-wrapper theme-default">
    ...
    <div id="slider" class="nivoSlider">
        ...
        while ( $query->have_posts() ) : $query->the_post();
          $i++;
          $natural_herbs_lite_slideno[] = $i;
          $natural_herbs_lite_slidetitle[] = get_the_title();
		  $natural_herbs_lite_slidedesc[] = get_the_excerpt();
          $natural_herbs_lite_slidelink[] = esc_url(get_permalink());
          ?>
        ...
        <?php
        $k = 0;
        foreach( $natural_herbs_lite_slideno as $natural_herbs_lite_sln ){ ?>
    <div id="slidecaption<?php echo esc_attr( $natural_herbs_lite_sln ); ?>" class="nivo-html-caption">
      <div class="slide_info">
        <h2><?php echo esc_html($natural_herbs_lite_slidetitle[$k] ); ?></h2>
        <?php if(!empty($natural_herbs_lite_slidedesc[$k])){?>
        <p><?php echo esc_html($natural_herbs_lite_slidedesc[$k] ); ?></p>
        <?php } ?>
        <div class="clear"></div>
        <a class="slide_more" href="<?php echo esc_url($natural_herbs_lite_slidelink[$k] ); ?>">
          <?php esc_html_e('Read More', 'natural-herbs-lite');?>
          </a>
      </div>
    </div>
 	<?php $k++;
...
<?php } ?>  
    ...
    <a class="featured-box-button" href="<?php echo esc_url( get_permalink() ); ?>"><?php esc_html_e('Read More', 'natural-herbs-lite');?></a> </div>
</div>
      <?php endwhile;
       wp_reset_postdata(); 
	   }} ?>
...
?>
 <section id="sec1section">
    ...
    <div class="home_section2_content">
            ...
            <div class="clear"></div>
            <div class="section2button"><a href="<?php echo esc_url( get_permalink() ); ?>"><?php esc_html_e('Read More', 'natural-herbs-lite');?></a></div>
	        <?php endwhile; } ?>       
            </div>  	
        ...
        <?php
                    if ( have_posts() ) :
						...
						the_posts_pagination( array(
							'mid_size' => 2,
							'prev_text' => esc_html__( 'Back', 'natural-herbs-lite' ),
							'next_text' => esc_html__( 'Next', 'natural-herbs-lite' ),
						) );
                    else :
